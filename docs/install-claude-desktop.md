...
Fully quit (cmd+Q on macOS) and relaunch. The three tools — `pay.discover`, `pay.fetch`, `pay.wallet_status` — should appear in the tool picker.

## 5. Verify

Ask Claude:

> *"Use the pay.wallet_status tool."*
...
**Tools appear but `pay.fetch` returns "no wallet configured":**
